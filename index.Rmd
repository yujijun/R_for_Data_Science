...
# 前言 {-}
大家好，我是于吉军,大家可以叫我小于；这里是小于《数据科学中的R语言》课程内容。很有幸能跟大家一起学习。

考虑到大家初入门，因此讲授的内容不会太深奥（要有信心喔）。
...
- 组学数据分析pipeline
   - 第 \@ref(RNAseq) 章介绍RNAseq数据处理的流程
   - 第 \@ref(downstream_analysis) 章介绍组学数据下游分析
   - 第 \@ref(scRNA-seq) 章介绍单细胞组学数据分析
   - 第 \@ref(dataset_down) 章介绍生物学数据下载
   - 第 \@ref(bioinformatics_info) 章介绍生物信息学分析中常用的资源链接

2、**课件源代码和数据**

  我将持续改进课件，所以欢迎大家提出建议
  
- <https://github.com/yujijun/R_for_Data_Science>
...
for (i in my_packages){
   ...
   if(!requireNamespace(i,quietly = TRUE)){
      install.packages(i,update = F,ask = F)
   }
}
...
knitr::include_graphics("images/weichatgroup.png")
```
...
非常感谢小于课题组小伙伴的信任，有了你们的支持，才会一起了解R的美！
